function dpPlotTrainSched(data, varToPlot, axisLim)

% Plots mean and sem data for experiment 'TrainSched' (Kristy's honors thesis project 2018-2019)
...
meanData = varfun(@nanmean, data, 'GroupingVariables', {'Group','abs_tgt_rot','TN_tgt'},'OutputFormat','table');
...
for ri = 1:length(rotations) ;
    for gi = unique(data.Group)';
        ...
        idx = meanData.Group == gi & abs(meanData.abs_tgt_rot) == rotations(ri);
        
        nanmeanVar = {strcat('nanmean_',varToPlot)};
        semVar = {strcat('sem_',varToPlot)};
        
        % Plot data
        x = meanData.TN_tgt(idx);
        y = meanData.(nanmeanVar{1})(idx);
